...
router.get("/singel/:id", async (req, res) => {
  ...
  try {
    const data = await ProductModel.findOne({ _id: id }, req.body);
    res.json(data);
  } catch (err) {
    console.log(err);
    res.status(502).json({ msg: "drink not found" });
  }
});
...
router.get("/price", async (req, res) => {
  ...
  try {
    const data = await ProductModel
      // יציג מוצרים שהמחיר שלהם גבוה מ 9
      // .find({price:{$gt:9}})
      // .find({price:{$gt:7,$lte:10}})

      // http://localhost:3033/products/price?max=500&min=300
      .find({ price: { $gt: min, $lte: max } });
    res.json(data);
  } catch (err) {
    console.log(err);
    res.status(502).json({ err });
...
// /products/search?s=sim
router.get("/search", async (req, res) => {
  const searchQ = req.query.s || "";
  try {
    const serachExp = new RegExp(searchQ, "i");
    // const data = await TvModel.find({name:serachExp})

    // http://localhost:3033/products?s=vans -> two products
    const data = await ProductModel.find({
      $or: [{ name: serachExp }, { info: serachExp }],
    });
    res.json(data);
  } catch (err) {
    console.log(err);
